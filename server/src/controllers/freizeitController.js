'use strict';
var Freizeit = require('../models/freizeitModel.js');
...
exports.set_to_stack = function (req, res) {
           ...
           if (err) {
            ...
            Freizeit.proof(stack, function (err, material) {
                ...
                if (typeof proof === "undefined"){
                    ...
                    console.log("update")
                    Freizeit.update(stack, function (err, material) {
                        if (err)
                            res.send(err);
                        res.send(material);
                    });
   ...
   Freizeit.getStackById(req.params.mmd_id, req.params.display, async function(err, stackData) {
        if (err)
            res.send(err);
        res.json(stackData);
        })
