'use strict';
var Heimat = require('../models/heimatModel.js');
    ...
    /*   ensureToken(req, res);
       jwt.verify(req.token, 'my_secret_key', function (err, data) {
           if (err) {
            ...
            Heimat.proof(stack, function (err, material) {
                if (err)
                    res.send(err);
                proof = material;
                console.log(proof)
                ...
                if (typeof proof === "undefined"){
                    console.log("init")
                    Heimat.setToStack(stack, function (err, material) {
                        if (err)
                            res.send(err);
                        res.send(material);
                    });
                    ...
                    console.log("update")
                    Heimat.update(stack, function (err, material) {
                        if (err)
                            res.send(err);
                        res.send(material);
                    });
